class Searcher {
	...
	init() {
		setTimeout(function() {
            let id, pac, arr = [];
            jQuery('.mvt-page-form-item-input').each((i,el) => {
                id = jQuery(el).attr('id');
                arr.push(id);
            });
            jQuery('.pac-container').each((i,el) => {
            	pac = jQuery(el);
                pac.wrap('<div class="mvt-'+arr[i]+'"/>');
                var searchList = jQuery('.mvt-'+arr[i]).detach();
                jQuery('#'+arr[i]+' .mvt-page-form-address-results').html(searchList);
            });
        }, 250);
	}
	...
	search(data) {
		...
		jQuery("#"+data.input).geocomplete({
			...
			location.lat = jQuery('#'+data.input).find('.mvt-page-form-address-lat').val();
			location.lon = jQuery('#'+data.input).find('.mvt-page-form-address-lon').val();
